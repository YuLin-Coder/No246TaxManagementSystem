    ...
    <link href="${pageContext.request.contextPath}/static/admin/css/public.css" rel="stylesheet" type="text/css">
        <link href="${pageContext.request.contextPath}/static/admin/css/modal_css.css" rel="stylesheet" type="text/css">
            <link rel="stylesheet" type="text/css" href="${pageContext.request.contextPath}/static/common/page/myPagination.css?t=4" />
                ...
                <select  id="cityId" style="height:30px;color:black;width:250px;" >
                ...
                <c:forEach items="${cityInfoList}" var="item">
                    <option value="${item.id}">${item.name}</option>
                        ...
                        <a href="${pageContext.request.contextPath}/admin/staff_info/add" class="btnGreen">新增</a>
                            ...
                            <tbody id="staff_info_body">
                            
                            </tbody>
                            </table>
                            <div id="staff_info_bar" class="pagination" style="margin-top:20px;"></div>
                            ...
                            <script type="text/javascript" src="${pageContext.request.contextPath}/static/common/jquery-1.11.1.min.js"></script>
                                <script type="text/javascript" src="${pageContext.request.contextPath}/static/common/page/myPagination.js"></script>
                                    <script type="text/javascript" src="${pageContext.request.contextPath}/static/admin/js/modal_js.js"></script>
                                        ...
                                        <script type="text/javascript" src="${pageContext.request.contextPath}/static/common/utils/listutils.js?v=5645"></script>
                                                ...
                                                <script type="text/javascript" src="${pageContext.request.contextPath}/static/common/selectSearch/jquery.searchableSelect.js?d=124393"></script>
                                                    ...
                                                    function ajaxList(page) {
                                                        ...
                                                        showLoading('${pageContext.request.contextPath}/static/common/loading.gif');
                                                            $.ajax({
                                                                type : 'get',
                                                                url : "${pageContext.request.contextPath}/admin/staff_info/queryList",
                                                                    data : {
                                                                    ...
                                                                    success : function(result) {
                                                                        ...
                                                                        for (var i = 0; i < rows.length; i++) {
                                                                            html += '<tr>' ;
                                                                            var loginNameVal=setNullToEmpty(rows[i].staffInfo.loginName);
                                                                            html+='<td>'+ loginNameVal+'</td>';
                                                                            var passwordVal=setNullToEmpty(rows[i].staffInfo.password);
                                                                            html+='<td>'+ passwordVal+'</td>';
                                                                            var realNameVal=setNullToEmpty(rows[i].staffInfo.realName);
                                                                            html+='<td>'+ realNameVal+'</td>';
                                                                            var celPhoneVal=setNullToEmpty(rows[i].staffInfo.celPhone);
                                                                            html+='<td>'+ celPhoneVal+'</td>';
                                                                            var idNumberVal=setNullToEmpty(rows[i].staffInfo.idNumber);
                                                                            html+='<td>'+ idNumberVal+'</td>';
                                                                            var cityIdVal=setNullToEmpty(rows[i].cityIdStr);
                                                                            html+='<td>'+ cityIdVal+'</td>';
                                                                            var addressVal=setNullToEmpty(rows[i].staffInfo.address);
                                                                            html+='<td>'+ addressVal+'</td>';
                                                                            var createTimeVal=setNullToEmpty(rows[i].staffInfo.createTime);
                                                                            html+='<td>'+ createTimeVal+'</td>';
                                                                            html+='<td>';
                                                                            html+='<a href="${pageContext.request.contextPath}/admin/staff_info/update?id='+rows[i].staffInfo.id+'" class="btn btnStyle">修改</a>';
                                                                                html+='<a href="javascript:void(0)" onclick="del(this)" data-id="'+rows[i].staffInfo.id+'" class="btnPayment btnStyle">删除</a>';
                                                                                html+='</td>';
                                                                                html+='</tr>';
                                                                            }
                                                                            if (rows.length == 0) {
                                                                ...
                                                                function del(e){
                                                                    if(window.confirm('你确定要删除该办税人员吗')){
                                                                        var id = $(e).attr("data-id");
                                                                        $.ajax({
                                                                            type : 'get',
                                                                            url : "${pageContext.request.contextPath}/admin/staff_info/del",
                                                                                data : {
                                                                                    "id":id
                                                                                },
                                                                                success : function(result) {

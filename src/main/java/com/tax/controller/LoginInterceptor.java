...
public class LoginInterceptor implements HandlerInterceptor {
    public boolean preHandle(HttpServletRequest request,
        ...
        if (contextPath2.equals("tax_sys") ||
                ((contextPath2.contains("admin") == false) &&
                (contextPath2.contains("staff") == false) &&
        ...
        if (login == null) {
            ...
            if (login.getLoginType() == 2) {
                if ((requestUri.contains("staff") == false) &&
                        (requestUri.contains("commonapi") == false)) { //当前登录角色为办税人员,但请求其他角色使用的接口,即盗链,这种需要拦截并提示错误
                    response.sendRedirect(contextPath +
                        "/commonapi/error?msg=-2");
